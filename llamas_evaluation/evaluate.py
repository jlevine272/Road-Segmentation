...
from .label_scripts import dataset_constants
from .common import helper_scripts
from .label_scripts import spline_creator
...
def evaluate(eval_file: str, split: str):
    ...
    for label in labels:
        ...
        if key not in regressions.keys():
            continue
        regression_lanes = regressions[key]
        for lane, lane_key in zip(spline_labels, ["l0", "r0"]):
            if len(regression_lanes[lane_key]) == 0:
                continue
